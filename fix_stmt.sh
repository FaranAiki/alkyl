#!/bin/bash
cat << 'PATCH' > stmt_patch.patch
--- src/parser/stmt.c
+++ src/parser/stmt.c
@@ -532,9 +532,46 @@ ASTNode* parse_single_statement_or_block_internal(Parser *p) {
 
   if (modifiers) parser_fail(p, "Invalid modifier on statement");
 
-  // if (p->current_token.type == TOKEN_IDENTIFIER) return parse_assignment_or_call(p);
   
   ASTNode *expr = parse_expression(p);
+  
+  if (expr && (expr->type == NODE_VAR_REF || expr->type == NODE_TEMPLATE_INSTANTIATION || expr->type == NODE_MEMBER_ACCESS)) {
+      TokenType t = p->current_token.type;
+      int is_arg_start = (t == TOKEN_NUMBER || t == TOKEN_SINGLE_LIT || t == TOKEN_STRING || 
+            t == TOKEN_C_STRING || t == TOKEN_BYTE_STRING ||
+            t == TOKEN_CHAR_LIT || t == TOKEN_TRUE || t == TOKEN_FALSE || 
+            t == TOKEN_IDENTIFIER || t == TOKEN_LPAREN || t == TOKEN_LBRACKET || 
+            t == TOKEN_NOT || t == TOKEN_BIT_NOT || t == TOKEN_MINUS || t == TOKEN_PLUS || t == TOKEN_STAR || t == TOKEN_AND || t == TOKEN_TYPEOF);
+      
+      if (is_arg_start) {
+          char *fname = NULL;
+          if (expr->type == NODE_VAR_REF) fname = ((VarRefNode*)expr)->name;
+          else if (expr->type == NODE_TEMPLATE_INSTANTIATION && ((TemplateInstNode*)expr)->target->type == NODE_VAR_REF) {
+              fname = ((VarRefNode*)((TemplateInstNode*)expr)->target)->name;
+          } else if (expr->type == NODE_MEMBER_ACCESS) {
+              fname = ((MemberAccessNode*)expr)->member_name;
+          }
+          
+          ASTNode *args_head = NULL;
+          ASTNode **curr_arg = &args_head;
+          
+          *curr_arg = parse_expression(p);
+          curr_arg = &(*curr_arg)->next;
+          
+          while (p->current_token.type == TOKEN_COMMA) { if (p->has_error) break;
+              eat(p, TOKEN_COMMA);
+              *curr_arg = parse_expression(p);
+              curr_arg = &(*curr_arg)->next;
+          }
+          
+          CallNode *cn = parser_alloc(p, sizeof(CallNode));
+          cn->base.type = NODE_CALL;
+          cn->name = fname;
+          cn->target = expr;
+          cn->args = args_head;
+          expr = (ASTNode*)cn;
+      }
+  }
+  
   eat_semi(p); 
   return expr;
 }
PATCH
patch src/parser/stmt.c stmt_patch.patch
make clean && make && ./test_ethyl_5.sh
