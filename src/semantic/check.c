...
void sem_check_call(SemanticCtx *ctx, CallNode *node) {
    ...
    if (!sym) {
        if (node->name) {
            ...
            printf("Cannot call non-function type at line %d col %d, node type %d, target type %d\n", node->base.line, node->base.col, node->base.type, node->target ? node->target->type : -1); sem_error(ctx, (ASTNode*)node, "Cannot call non-function type");
        }
