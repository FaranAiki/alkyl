...
void alir_stmt_vardecl(AlirCtx *ctx, ASTNode *node) {
    ...
    if (vn->initializer && vn->initializer->type == NODE_CALL) {
     ...
     if (is_stack_ctor == 2) {
        ...
        emit(ctx, mk_inst(ctx->module, ALIR_OP_STORE, NULL, arg_val, ptr));
        return;
    ...
    if (vn->initializer) {
        ...
        if (val) {
            ...
            if (vn->var_type.is_tainted && val->type.base == TYPE_VOID && val->type.ptr_depth == 1 && val->kind == ALIR_VAL_CONST && val->val.long_val == 0) {
                int err_id = 1;
                ...
                AlirValue *val_ptr = new_temp(ctx, (VarType){TYPE_INT, 1, NULL, 0, 0, NULL, NULL, 0, 0, 0, 0});
                ...
                VarType raw_type = vn->var_type;
                raw_type.is_tainted = 0;
                zero_val = promote(ctx, zero_val, raw_type);
...
void alir_stmt_assign(AlirCtx *ctx, ASTNode *node) {
    ...
    if (ptr) {
        ...
        if (target_type.is_tainted && val->type.base == TYPE_VOID && val->type.ptr_depth == 1 && val->kind == ALIR_VAL_CONST && val->val.long_val == 0) {
            int err_id = 1;
            if (ctx->sem && ctx->sem->compiler_ctx) {
                void *val_err = hashmap_get(&ctx->sem->compiler_ctx->error_table, "ErrNull");
                if (val_err) err_id = (int)(intptr_t)val_err;
            ...
            AlirValue *err_ptr = new_temp(ctx, (VarType){TYPE_INT, 1, NULL, 0, 0, NULL, NULL, 0, 0, 0, 0});
            emit(ctx, mk_inst(ctx->module, ALIR_OP_GET_PTR, err_ptr, ptr, alir_const_int(ctx->module, 0)));
            emit(ctx, mk_inst(ctx->module, ALIR_OP_STORE, NULL, alir_const_int(ctx->module, err_id), err_ptr));
            
            AlirValue *val_ptr = new_temp(ctx, (VarType){TYPE_INT, 1, NULL, 0, 0, NULL, NULL, 0, 0, 0, 0});
            emit(ctx, mk_inst(ctx->module, ALIR_OP_GET_PTR, val_ptr, ptr, alir_const_int(ctx->module, 1)));
            
            AlirValue *zero_val = alir_const_int(ctx->module, 0);
            VarType raw_type = target_type;
            raw_type.is_tainted = 0;
            zero_val = promote(ctx, zero_val, raw_type);
            emit(ctx, mk_inst(ctx->module, ALIR_OP_STORE, NULL, zero_val, val_ptr));
            return;
