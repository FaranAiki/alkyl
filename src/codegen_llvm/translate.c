    ...
    switch (inst->op) {
        ...
        case ALIR_OP_FALLBACK: {
            ...
            if (inst->arg_count > 0 && inst->args[0]) {
                ...
                // If it is the target error, we return the fallback value.
                // But wait! If it's NOT the target error (but it might still be an error), we need to return the original tainted struct!
                // So res needs to be of the same type as op1, NOT op2!
                
                // Wait, if it has a target err, we return a new tainted struct:
                // Wait! If it's the target err, the error is cleared (err_id = 0), and val = op2.
                // If it's NOT the target err, it remains as is.
                // So we can select err_id = has_err ? 0 : err_id.
                // And val = has_err ? op2 : val.
                // Then construct the new tainted struct.
        ...
        case ALIR_OP_ITER_INIT: {
            codegen_llvm_flux_iter_init(ctx, inst, op1, &res);
            break;
        }
        case ALIR_OP_ITER_VALID: {
            codegen_llvm_flux_iter_valid(ctx, op1, &res);
            break;
        }
        case ALIR_OP_ITER_GET: {
            codegen_llvm_flux_iter_get(ctx, inst, op1, &res);
            break;
        }
        case ALIR_OP_ITER_NEXT: {
            codegen_llvm_flux_iter_next(ctx, op1); 
            break;
        }
