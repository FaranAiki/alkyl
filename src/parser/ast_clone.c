    ...
    if (t.base == TYPE_CLASS && t.class_name) {
        ...
        if (bracket) {
            ...
            for (int i = 0; i < num_renames; i++) {
                if (streq_lit(base_name, rename_from[i])) {
            ...
            if (array_size > 0) new_t.array_size = array_size;
            debug_parser("clone_var_type bracket: %s -> %s array_size=%d\n", t.class_name, new_t.class_name, new_t.array_size);
            return new_t;
        }

        for (int i = 0; i < num_params; i++) {
            if (streq_lit(t.class_name, type_params[i])) {
                VarType new_t = replace_with[i];
                new_t.ptr_depth += t.ptr_depth;
        ...
        for (int i = 0; i < num_renames; i++) {
            if (streq_lit(t.class_name, rename_from[i])) {
                VarType new_t = t;
...
ASTNode* ast_clone(CompilerContext *ctx, ASTNode *node, char **type_params, VarType *replace_with, int num_params, char **rename_from, char **rename_to, int num_renames) {
    ...
    switch (node->type) {
        ...
        case NODE_VAR_REF: {
            ...
            *n = *orig;
            if (orig->name) {
                ...
                for (int i = 0; i < num_renames; i++) {
                    if (streq_lit(orig->name, rename_from[i])) {
            ...
            *n = *orig;
            if (orig->name) {
                ...
                if (strncmp(orig->name, "as_", 3) == 0) {
                    for (int i = 0; i < num_params; i++) {
                        if (streq_lit(orig->name + 3, type_params[i])) {
                            char buf[256];
            ...
            if (orig->class_name) {
                ...
                for (int i = 0; i < num_renames; i++) {
                    if (streq_lit(orig->class_name, rename_from[i])) {
        ...
        case NODE_FOR_IN: {
            ForInNode *orig = (ForInNode*)node;
            ForInNode *n = arena_alloc(ctx->arena, sizeof(ForInNode));
            *n = *orig;
            n->collection = ast_clone(ctx, orig->collection, type_params, replace_with, num_params, rename_from, rename_to, num_renames);
            n->body = ast_clone(ctx, orig->body, type_params, replace_with, num_params, rename_from, rename_to, num_renames);
            clone = (ASTNode*)n;
            break;
        }
...
ASTNode* ast_rewrite_macro(CompilerContext *ctx, ASTNode *node, ASTNode *varargs_head, char **param_names, ASTNode **param_args, int num_params) {
    ...
    if (node->type == NODE_VAR_REF) {
        ...
        for (int i=0; i<num_params; i++) {
            if (param_names[i] && streq_lit(vn->name, param_names[i])) {
    ...
    switch (node->type) {
        ...
        case NODE_FOR_IN: {
            ...
            if (fn->collection && fn->collection->type == NODE_VAR_REF && ((VarRefNode*)fn->collection)->name && streq_lit(((VarRefNode*)fn->collection)->name, "...")) {
                ...
                while (curr_arg) {
                    ...
                    ASTNode **new_args = arena_alloc(ctx->arena, sizeof(ASTNode*) * (num_params + 1));
                    for (int i=0; i<num_params; i++) {
                        new_names[i] = param_names[i];
                        new_args[i] = param_args[i];
                    }
