...
ASTNode* parse_assignment_or_call(Parser *p) {
  ...
  if (node->type == NODE_VAR_REF || node->type == NODE_TEMPLATE_INSTANTIATION) {
      ...
      int is_arg_start = (t == TOKEN_NUMBER || t == TOKEN_SINGLE_LIT || t == TOKEN_STRING || 
            ...
            t == TOKEN_CHAR_LIT || t == TOKEN_TRUE || t == TOKEN_FALSE || t == TOKEN_NULL ||
...
ASTNode* parse_single_statement_or_block_internal(Parser *p) {
  ...
  // := shorthand for define ... as ...
  if (p->current_token.type == TOKEN_IDENTIFIER && parser_peek_token(p).type == TOKEN_WALRUS) {
      if(modifiers) { parser_fail(p, "Modifiers not allowed on ':=', use 'define' instead"); return NULL; }
